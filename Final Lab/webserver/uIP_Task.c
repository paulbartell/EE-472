...
{
  xSemaphoreTake(*(remoteData.commandData->commandSemaphorePtr),200);
  char* cmd;
  char* text;
  short err = 0;
  ...
  for(int i = 0; i < 4; i++)
  {
    remoteData.commandData->measure[i] = 0;
  }
  cmd = strstr(pcInputString,"m=");
  if (cmd) cmd += strlen("m=");
  ...
  while(cmd != NULL)
  {
    ...
    {
    case 'T' :
      remoteData.commandData->measure[0] = 1;
      break;
    case 'B' :
      remoteData.commandData->measure[1] = 1;
      break;
    case 'P' :
      remoteData.commandData->measure[2] = 1;
      break;
    case 'E' :
      remoteData.commandData->measure[3] = 1;
      break;
    default:
      err = 1;
      break;
  ...
  remoteData.commandData->recieve = cmd[0];
  cmd = NULL;
  /* Find the start of the text to be displayed on the LCD. */
  text = strstr( pcInputString, "do=" );
  if (text) {
    remoteData.commandData->recieve = 'S';
    text += strlen( "do=" );
    ...
    *cmd = 0x00;
    ...
    /* Add required spaces. */
    while( ( cmd = strstr( text, "+" ) ) != NULL )
    {
      *cmd = ' ';
    }
    cmd = strstr(text, "&p=");
    *cmd = '\0';
    strcpy(remoteData.docName, text);
    text = cmd + strlen("&p=");
    strcpy(remoteData.patName, text);
  }
